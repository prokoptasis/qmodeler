...
namespace DocToolkit
{
    ...
    public class PersistWindowState
    {
		...
		// 추가한 부분
		private int gridpointx = 1;
		private int gridpointy = 1;
        ...
        public PersistWindowState(string path, Form owner)
        {
			...
			if(key != null)
			{
				if(key.GetValue("GridPointX") != null)
					this.GridpointX = (int)key.GetValue("GridPointX");

				if(key.GetValue("GridPointY") != null)
					this.GridpointY = (int)key.GetValue("GridPointY");
			}
        }
        ...
        private void OnClosing(object sender, System.ComponentModel.CancelEventArgs e)
        {
			...
			// 추가한 부분
			key.SetValue("GridPointX", gridpointx);
			key.SetValue("GridPointY", gridpointy);
